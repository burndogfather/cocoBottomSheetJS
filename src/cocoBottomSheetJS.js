...
class cocoButtomSheetJS{
	constructor({
		...
		this.BSElement.innerHTML = '<button class="cocoButtomSheet_handle"></button><div class="cocoButtomSheet_body">'+ this.code +'</div>';
	...
	init(){
		...
		this.BSElement.querySelector('.cocoButtomSheet_body').style.height = Math.floor(this.max - ( parseFloat(getComputedStyle(document.documentElement).fontSize) * 3) )+'px'; 
